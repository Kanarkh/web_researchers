...
<form class="ui form" method="post" action="./post/signup_action.php">
  ...
  <div class="field">
    ...
    <div class="ui grid">
      <div class="twelve wide column">
        <input type="email" id="email" name="email" placeholder="이메일을 입력해주세요" onchange="emailChange();">
      </div>
      <div class="four wide column">
        <button class="ui fluid primary button" type="button" onClick="emailCheck();">
        중복검사
        </button>
      </div>
    </div>
  ...
  <div class="field">
    ...
    <div class="ui grid">
      <div class="twelve wide column">
        <input type="text" maxlength="10" id="nickname" name="nickname" placeholder="닉네임을 입력해주세요" onchange="nicknameChange();">
      </div>
      <div class="four wide column">
        <button class="ui fluid primary button" type="button" onclick="nicknameCheck();">
        중복검사
        </button>
      </div>
    </div>
...
<div class="two fields" align=left>
  ...
  <!-- checkbox -->
  <div class="field" align=left style="padding-top:20px;">
    <div class="ui checkbox" >
      <input type="checkbox" tabindex="0" name="checkPrivacyPolicy" class="hidden" >
      <label ><a href="index.php?where=PrivacyPolicy" target="_blank">개인정보 취급방침</a>을 확인하였으며 동의합니다. (필수)</label>
    </div>
  </div>
  <div class="field" align=left>
    <div class="ui checkbox">
      <input type="checkbox" tabindex="1" name="checkTerms" class="hidden">
      <label><a href="index.php?where=TermsOfUse" target="_blank">이용약관</a>을 확인하였으며 동의합니다. (필수)</label>
    </div>
  </div>
  <input type="hidden" id="hiddenemail" name="hiddenemail" value="">
  <input type="hidden" id="hiddenenickname" name="hiddenenickname" value="">
  <input type="hidden" id="hiddenpwck" name="hiddenpwck" value="">
  <button class="ui primary button" type="submit" style="width:650px">회원가입</button>
...
<script>
    function emailCheck(){
      ...
      var check = document.getElementById("hiddenemail");
    
      var mEmail = document.getElementById("email");
      var evalue =mEmail.value;

      $.ajax({
      url: "../ajax/signupcheck.php", // 클라이언트가 HTTP 요청을 보낼 서버의 URL 주소
      data: { type: "email", value: evalue},                         // HTTP 요청과 함께 서버로 보낼 데이터
      type: "POST",                             // HTTP 요청 방식(GET, POST)

      success: function($result){
        if($result.match(/ok/)){
          check.value ='ok';
          alert("사용할수있는 email입니다");
        }else if($result.match(/no/)){
          check.value ='';
          alert("이미 사용중인 email입니다 다른 email을 입력해주세요");
        }
      }
      })
    ...
    function nicknameCheck(){
      ...
      var check = document.getElementById("hiddenenickname");
      var mEmail = document.getElementById("nickname");
      var nvalue =mEmail.value;
      $.ajax({
      url: "../ajax/signupcheck.php", // 클라이언트가 HTTP 요청을 보낼 서버의 URL 주소
      data: { type: "nickname", value: nvalue},                         // HTTP 요청과 함께 서버로 보낼 데이터
      type: "POST",                             // HTTP 요청 방식(GET, POST)
    ...
    function emailChange(){
      var ech = document.getElementById("hiddenemail");
      ech.value ='';
    }
    function nicknameChange(){
      var ech = document.getElementById("hiddenenickname");
      ech.value ='';
    }
    function pwChange(){
    ...
    fields: {
      email: {
        ...
        rules: [
          {
            type   : 'email',
            prompt : 'email을 입력해주세요'
          }
        ]
      },
      name2: {
        identifier: 'hiddenemail',
        rules: [
          {
            type   : 'empty',
            prompt : 'email 중복확인을 해주세요'
          }
        ]
      },
      name: {
        ...
        rules: [
          {
            type   : 'empty',
            prompt : 'nickname을 입력해주세요'
          }
        ]
      },
      name3: {
        identifier: 'hiddenenickname',
        rules: [
          {
            type   : 'empty',
            prompt : 'nickname 중복확인을 해주세요'
          }
        ] 
      },
      password: {
      ...
      name4: {
        ...
        rules: [
          {
            type   : 'empty',
            prompt : '비밀번호가 비밀번호 확인과 다릅니다 확인해주세요'
          }
        ] 
      },
      minCount: {
      ...
      terms: {
        identifier: 'checkPrivacyPolicy',
        rules: [
          {
            type   : 'checked',
            prompt : '개인정보 취급방침을 동의해주세요'
          }
        ]
      },
      terms1: {
        identifier: 'checkTerms',
        rules: [
          {
            type   : 'checked',
            prompt : '이용약관을 동의해주세요'
